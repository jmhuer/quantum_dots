...
\subsection{Applications of Multiplex Detection}
...
\subsection{Machine learning for Assays Predictions}
I think here is where we discuss background on ML and assays predictions and the
benefit in using raspberry pi vs smart phone 


\section{Data collections and Experimental details }
...
strength symbolized as $\mu _{0}H$. Use the center dot to separate 
compound units, e.g., ``A$\cdot $m$^{2}$.''. 
Here we refer to the table of models 
<TABLE OF MODELS> 
...
Here we discuss corss validation accruacy and BIC, AIC criterial? 
